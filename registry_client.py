...
logger = get_logger("doctor.registry")

SERVICE_NAME = "doctor"
SERVICE_VERSION = "0.1.0"
SERVICE_CAPABILITIES = ["diagnostics", "health_report"]
...
class RegistryClientConfig:
    ...
    def from_env(cls) -> RegistryClientConfig:
        return cls(
            ...
            service_port=int(os.getenv("NERON_SERVICE_PORT", "8020")),
        )


class RegistryClient:
    """Non-fatal Doctor bootstrap client for the Core Service Registry."""
    ...
    async def start(self) -> None:
        if self._heartbeat_task is None:
            self._heartbeat_task = asyncio.create_task(
                self._run(),
                name="doctor-registry-heartbeat",
            )
    ...
    async def register(self) -> bool:
        ...
        logger.info(
            "Registered Doctor with Core Registry at %s",
    ...
    async def heartbeat(self) -> bool:
        ...
        logger.debug("Doctor Registry heartbeat sent")
        return True
